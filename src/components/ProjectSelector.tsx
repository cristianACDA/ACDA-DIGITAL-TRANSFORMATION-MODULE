...
export default function ProjectSelector() {
  ...
  if (loading) {
    return (
      <div className="bg-white border border-[color:var(--color-border-subtle)] rounded-lg p-8 text-center shadow-sm">
        <p className="text-sm text-[color:var(--color-text-body)]/60">Se încarcă proiectele…</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="bg-[color:rgba(245,158,11,0.08)] border border-border-subtle rounded-lg p-6">
        <p className="text-sm text-accent-warning font-semibold mb-1">Eroare la încărcare</p>
        <p className="text-xs text-accent-warning/80">{error}</p>
      </div>
    )
  }
  ...
  if (projects.length === 0) {
    return (
      <div className="bg-white border border-[color:var(--color-border-subtle)] rounded-lg p-8 text-center shadow-sm">
        ...
        <h2 className="text-xl font-semibold text-[color:var(--color-text-primary)] mb-2">Niciun proiect</h2>
        <p className="text-sm text-[color:var(--color-text-body)]/60">
          Rulează <code className="bg-[color:var(--color-page)] px-1.5 py-0.5 rounded text-xs">npm run db:init</code> pentru a popula cu seed.
        </p>
      </div>
    )
  }
  ...
  return (
    <section className="bg-white border border-[color:var(--color-border-subtle)] rounded-lg shadow-sm overflow-hidden">
      <div className="px-5 py-3 border-b border-[color:var(--color-border-subtle)] flex items-center justify-between">
        <h2 className="text-xs font-semibold text-[color:var(--color-text-body)]/50 uppercase tracking-widest">
          Proiecte ({projects.length})
        </h2>
        <span className="text-xs text-[color:var(--color-text-body)]/40">Click pentru a încărca</span>
      </div>
          ...
          return (
            <li key={p.id}>
              <button
                ...
                className="w-full text-left px-5 py-4 hover:bg-[color:var(--color-page)] transition-colors flex items-center gap-4"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-bold text-[color:var(--color-text-body)] truncate">
                      {p.client_company_name ?? '—'}
                    </span>
                    <span className="text-xs font-mono bg-[color:var(--color-page)] border border-[color:var(--color-border-subtle)] text-[color:var(--color-text-body)]/50 px-1.5 py-0.5 rounded">
                      {p.status}
                    </span>
                  </div>
                  <p className="text-xs text-[color:var(--color-text-body)]/60 truncate">{p.name}</p>
                </div>
                  ...
                  {info.hasScores ? (
                    <>
                      <div className="flex items-baseline gap-1 justify-end">
                        <span className={`text-2xl font-semibold tabular-nums ${cfg.text}`}>
                    ...
                    <span className="text-xs text-[color:var(--color-text-body)]/40">fără scor</span>
                  )}
                </div>

                <span className="text-[color:var(--color-text-body)]/30 text-lg flex-shrink-0">›</span>
              </button>
            </li>
          )
        })}
