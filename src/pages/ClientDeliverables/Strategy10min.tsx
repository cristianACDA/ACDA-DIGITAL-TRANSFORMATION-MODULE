  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 flex flex-col gap-6">
      ...
      <div className="flex items-start justify-between gap-4 pb-4 border-b border-[#E6E6E6]">
        <div>
          <p className="text-xs text-[#071F80]/70 uppercase tracking-widest font-semibold">Strategie de Transformare</p>
          <h1 className="text-2xl font-black text-[#071F80] mt-1">{data.clientName}</h1>
      ...
      <Chapter num={2} title="Costul inacţiunii" text={capitol2} highlight={
        <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm">
          <strong className="text-red-700">Pierderi estimate pe 12 luni: {MS_FORMAT(data.pierderiAnuale)} RON</strong>
          <span className="text-[#0A2540]/70"> · Delta EBIT lăsat pe masă: {MS_FORMAT(data.deltaEbit)} RON</span>
        </div>
      } />
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="text-[15px] leading-relaxed text-[#0A2540]">{capitol3Intro}</p>
        <div className="grid gap-3 md:grid-cols-3">
          {pillars.map((p) => (
            <div key={p.titlu} className="bg-[#F6F9FC] border border-[#E6E6E6] rounded-xl p-4">
              <p className="text-xs text-[#071F80]/70 uppercase tracking-widest font-semibold">{p.titlu.split(':')[0]}</p>
              <h3 className="text-base font-bold text-[#071F80] mt-1 mb-2 leading-snug">{p.titlu.split(':').slice(1).join(':').trim()}</h3>
              <p className="text-xs text-[#0A2540]/80"><strong>{p.impact}</strong></p>
              <p className="text-xs text-[#0A2540]/60 mb-2">{p.efort}</p>
              <p className="text-xs text-[#0A2540]/80 leading-relaxed">{p.narativ}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="flex flex-col gap-4">
        ...
        <p className="text-[15px] leading-relaxed text-[#0A2540]">{capitol4Intro}</p>
            ...
            <div className="relative grid" style={{ gridTemplateColumns: `repeat(${milestones.length}, 1fr)` }}>
              {milestones.map((m, i) => (
                <div key={i} className="flex flex-col items-center">
                  <div className="text-[10px] font-bold uppercase tracking-wider text-[#071F80] mb-1">{m.when}</div>
                  <div className="w-4 h-4 rounded-full bg-[#071F80] border-2 border-white shadow-md z-10" />
                  <div className="text-xs text-center mt-2 text-[#0A2540] px-1 max-w-[140px]">{m.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Primul pas */}
        <div className="bg-[#2E75B6] text-white rounded-xl p-5">
      ...
      {/* Footer */}
      <div className="text-center text-xs text-[#0A2540]/50 border-t border-[#E6E6E6] pt-4 mt-4">
...
function ChapterHeader({ num, title }: { num: number; title: string }) {
  return (
    <div>
      ...
      <h2 className="text-2xl font-black text-[#071F80] mt-1">{title}</h2>
...
function Chapter({
  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      {text.split('\n\n').map((p, i) => (
        <p key={i} className="text-[15px] leading-relaxed text-[#0A2540]">{p.trim()}</p>
      ))}
      {highlight}
    </section>
  )
